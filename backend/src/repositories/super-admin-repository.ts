...
import {
  ...
  mockPlantProgress,
  mockRecipientVerifications,
} from "@/repositories/mock-storage";
import type {
  EscortRequest,
  EscortStatus,
...
interface SuperAdminDataState {
  recipients: RecipientVerification[];
  plantReports: PlantConditionReport[];
  harvestVerifications: HarvestVerification[];
  escortRequests: EscortRequest[];
}

const STORAGE_FILE = join(process.cwd(), process.env.UPLOAD_DIR ?? "uploads", "super-admin-data.json");

const clone = <T>(records: T[]): T[] => records.map((item) => (typeof item === "object" ? { ...(item as object) } : item)) as T[];
...
export class SuperAdminRepository {
  ...
  private async readState(): Promise<SuperAdminDataState> {
    try {
      const raw = await fs.readFile(STORAGE_FILE, "utf-8");
      const parsed = JSON.parse(raw) as Partial<SuperAdminDataState>;
      return {
  ...
  private async writeState(state: SuperAdminDataState) {
  ...
  async updateRecipient(id: string, payload: RecipientVerificationPayload): Promise<RecipientVerification> {
    ...
    const updated: RecipientVerification = {
      ...current,
  ...
  async createPlantReport(payload: PlantConditionPayload): Promise<PlantConditionReport> {
    ...
    const record: PlantConditionReport = {
      id: uuid(),
  ...
  async updatePlantReport(id: string, payload: Partial<PlantConditionPayload>): Promise<PlantConditionReport> {
    ...
    const updated: PlantConditionReport = {
      ...current,
  ...
  async updateHarvestVerification(id: string, payload: Partial<HarvestVerificationPayload>): Promise<HarvestVerification> {
    const state = await this.readState();
    ...
    const updated: HarvestVerification = {
      ...current,
      petani: payload.petani ?? current.petani,
      komoditas: payload.komoditas ?? current.komoditas,
  ...
  async updateEscortRequest(id: string, payload: Partial<EscortRequestPayload> & { status?: EscortStatus }): Promise<EscortRequest> {
    const state = await this.readState();
